...
func buildTestRequest(channelType, baseURL, modelName, endpointType string, maxTokens int) (string, map[string]interface{}) {
	...
	if !hasVersionSegment(base) {
		versionedBase = base + "/v1"
	}

	switch channelType {
	case model.ChannelTypeAnthropic:
		return versionedBase + "/messages", map[string]interface{}{
			"model":      modelName,
			"messages":   []map[string]string{{"role": "user", "content": "hi"}},
			"max_tokens": maxTokens,
		}
	case model.ChannelTypeResponses:
		return versionedBase + "/responses", map[string]interface{}{
			"model":             modelName,
			"input":             "hi",
	...
	default:
		switch endpointType {
		case "embedding":
			return versionedBase + "/embeddings", map[string]interface{}{
				"model": modelName,
				"input": "hello",
			}
		default:
			return versionedBase + "/chat/completions", map[string]interface{}{
				"model":      modelName,
				"messages":   []map[string]string{{"role": "user", "content": "hi"}},
				"max_tokens": maxTokens,
				"stream":     false,
